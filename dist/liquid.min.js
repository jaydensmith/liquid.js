...
return output;});},assertMissingDelimitation:function(){throw(this.blockName+" tag was never closed");},whitespaceHandler:function(token,tokens){if(token[2]=='-'){if(this.nodelist.length>0&&this.nodelist[this.nodelist.length-1].trim()===''){this.nodelist.pop();}}
if(token[token.length-3]==='-'){if(tokens.length>0&&tokens[0].trim()===''){tokens.shift();}}}});Liquid.Document=Liquid.Block.extend({init:function(tokens){this.blockDelimiter=[];this.parse(tokens);},assertMissingDelimitation:function(){}});Liquid.Strainer=Liquid.Class.extend({init:function(context){this.context=context;},respondTo:function(methodName){methodName=methodName.toString();if(methodName.match(/^__/))return false;if(Liquid.extensions.arrayTools.include(Liquid.Strainer.requiredMethods,methodName))return false;return(methodName in this);}});Liquid.Strainer.filters={};Liquid.Strainer.globalFilter=function(filters){for(var f in filters){Liquid.Strainer.filters[f]=filters[f];}}
